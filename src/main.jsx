...
function App() {
  ...
  return <main>
    ...
    <section className="grid two"><ChartCard title="Inference confidence"><ResponsiveContainer width="100%" height={300}><ScatterChart><CartesianGrid /><XAxis dataKey="x" name="entry" /><YAxis dataKey="y" name="confidence" domain={[40, 100]} /><Tooltip cursor={{ strokeDasharray: '3 3' }} formatter={(value, name, props) => [`${value}%`, props.payload.name]} /><Scatter data={scatter} /></ScatterChart></ResponsiveContainer></ChartCard><section className="card methodology"><h3>Methodology</h3><p><strong>High confidence:</strong> explicit country, nationality, flag, language, or named folklore tradition.</p><p><strong>Medium confidence:</strong> strong cultural form such as samba, salsa, cumbia, capoeira, flamenco, bhangra, or regional music/dance.</p><p><strong>General tag:</strong> rows tagged <code>General</code> have no reliable nationality match after reviewing the source description.</p><p><strong>Multi-tag entries:</strong> styles with layered roots are counted across all relevant tags. For example, Salsa Caleña can count as Colombia and Caribbean; Salsa Cubana can count as Cuba and Caribbean.</p><p>Broad regional labels are kept only where the source does not give enough country signal.</p></section></section>
